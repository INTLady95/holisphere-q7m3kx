...
// ===================== "grab and drag" hint: only in rooms where you can look around =====================
let dragTimer = null;
function showDragHint(on) {
  const h = $("drag-hint"); clearTimeout(dragTimer); h.classList.remove("show");
  if (!on || onb.active) return;
  h.querySelector("span").textContent = L.ui.dragHint; h.classList.add("show");
  // the camera itself nods left and right so the visitor sees what "drag" does
  const b = tour.base, lk = tour.look, y0 = b.yaw;
  gsap.timeline().to(b, { yaw: y0 - 0.22, duration: 0.9, ease: "sine.inOut", onUpdate: () => { lk.yaw = b.yaw; } }, 0.5)
    .to(b, { yaw: y0 + 0.22, duration: 1.4, ease: "sine.inOut", onUpdate: () => { lk.yaw = b.yaw; } })
    .to(b, { yaw: y0, duration: 0.9, ease: "sine.inOut", onUpdate: () => { lk.yaw = b.yaw; } });
  dragTimer = setTimeout(() => h.classList.remove("show"), 4200);
}
